...
def debug_output(instruction, line):
    output = []
    ...
    print_line_from_stack("Scalar Stack", SCALAR_STACK)
    print_line_from_stack("ARRAY Stack", ARRAY_STACK)
    output.append("")
    ...
    def get_value(self):
        if self.name not in SYMBOL_TABLE:
            raise InterpreterError(
                "{} not in symbol table. Needs to be initialized.".format(
                    self.name))
...
def execute_bad_instruction(instruction):
    ...
    if command == "push":
        assert not isinstance(instruction[1], ArrayVar)
        a = instruction[1].get_value()
    ...
    if command == "ar_push":
        assert isinstance(instruction[1], ArrayVar)
        source_array = instruction[1].get_value()
        ARRAY_STACK.append(copy.deepcopy(source_array))
        return
...
def execute_bad_instructions(instructions):
    ...
    while instruction_pointer < len(instructions):
        ...
        if instruction:
            ...
            if DEBUG_MODE:
                debug_output(instruction, instruction_pointer)
